<?php
/**
 * Die Datei enthält die Klasse "{@link Template_AppStore}"
 *
 * @category Heimdallr
 * @package Heimdallr
 * @subpackage Template
...
/**
 * Template für die Preis-Entwicklung vom AppStore
 *
 * @category Heimdallr
 * @package Heimdallr
 * @subpackage Template
 ...
 * @author Kjell Dießel
 */
class Template_AppStore extends Heimdallr_TemplateAbstract
{

    const VALUE_PRICE = 'appstore';
    ...
    {
        $title = $this->getServer()->getGraphTitle('AppStore-Preis von', '');
        $label = '€';
        $name  = $this->adjustText(array(
            self::VALUE_PRICE => 'Preis',
        ));
        $args = array(
            'DEF:g1ave='.$this->getRrdtool()->getPathDatabase().':'.self::VALUE_PRICE.':'.Heimdallr_Rrdtool::RRD_TYPE_AVERAGE,
            'DEF:g1max='.$this->getRrdtool()->getPathDatabase().':'.self::VALUE_PRICE.':'.Heimdallr_Rrdtool::RRD_TYPE_MAX,
            'DEF:g1min='.$this->getRrdtool()->getPathDatabase().':'.self::VALUE_PRICE.':'.Heimdallr_Rrdtool::RRD_TYPE_MIN,
            'AREA:g1ave'.Heimdallr_Rrdtool::COLOR_DEFAULT.Heimdallr_Rrdtool::TEXT_FORMAT_ALPHA,
            'LINE1:g1ave'.Heimdallr_Rrdtool::COLOR_DEFAULT.':"'.$name[self::VALUE_PRICE].'"',
